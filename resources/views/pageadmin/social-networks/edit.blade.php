...
<div class="container-fluid">
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
                    ...
                    <div class="col-md-6">
                        <div class="form-group">
                            ...
                            <input type="url" class="form-control @error('url') is-invalid @enderror" 
                                   id="url" name="url" value="{{ old('url', $socialNetwork->url) }}" required>
                            @error('url')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <input type="number" min="0" class="form-control @error('order') is-invalid @enderror" 
                                   id="order" name="order" value="{{ old('order', $socialNetwork->order) }}">
                            @error('order')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
                        </div>
                    ...
                    <div class="col-md-3">
                        <div class="form-group">
                            ...
                            <select class="form-control @error('active') is-invalid @enderror" 
                                ...
                                <option value="1" {{ old('active', $socialNetwork->active) ? 'selected' : '' }}>Activo</option>
                                <option value="0" {{ !old('active', $socialNetwork->active) ? 'selected' : '' }}>Inactivo</option>
                            </select>
                            @error('active')
                                <div class="invalid-feedback">{{ $message }}</div>
                            @enderror
            ...
            <form action="{{ route('social-networks.destroy', [$client, $socialNetwork]) }}" method="POST" class="mt-3">
                @csrf
                @method('DELETE')
